...
public class UtilFileUpload {
	 ...
	 * you can get the byte[] from the map using key "nm[]" and the content type
	 * from the "nm". Note that the request is consumed by this method. This
	 * method depends on commons fileupload.
	 */
	...
	public static String get(Map<String, Object> params, String paramName,
			String dfltValue) {
		Object rv = params.get(paramName);
		if (rv != null) {
			String rvString = rv.toString();
			if (rvString.length() > 0)
				return rvString;
		}
		return dfltValue;
	}

	public static double get(Map<String, Object> params, String paramName,
			double dfltValue) {
		String vl = get(params, paramName, "");
		try {
			if (vl.length() > 0)
	...
	public static long get(Map<String, Object> params, String paramName,
			long dfltValue) {
		String vl = get(params, paramName, "");
		try {
			if (vl.length() > 0)
	...
	public static byte[] get(Map<String, Object> params, String paramName,
			byte[] dfltValue) {
		Object rv = params.get(paramName);
	...
	public static boolean get(Map<String, Object> params, String paramName,
			boolean dfltValue) {
		String vl = get(params, paramName, "");
		if ("true".equals(vl))
			return true;
